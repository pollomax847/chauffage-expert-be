...
class PACPage extends ConsumerStatefulWidget {
  const PACPage({super.key});

  @override
  ConsumerState<PACPage> createState() => _PACPageState();
}

class _PACPageState extends ConsumerState<PACPage> {
  ...
  void _calculer() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      setState(() {
        _resultat = CollectifService.calculerPuissancePAC(
          ...
          temperatureEau: _temperatureEau,
        );
      });
    }
  }
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      'Dimensionnement de la pompe à chaleur collective',
                    ...
                    TextFormField(
                      decoration: const InputDecoration(
                        labelText: 'Température de l\'eau (°C)',
                        border: OutlineInputBorder(),
                      ),
                      keyboardType: TextInputType.number,
                      validator: (value) {
                      ...
                      onSaved: (value) {
                        _temperatureEau = double.parse(value!);
                      },
                    ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Puissance de base : ${_resultat!['puissanceBase']?.toStringAsFixed(1) ?? '0'} W/m²',
                        style: Theme.of(context).textTheme.bodyLarge,
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Coefficient température : ${_resultat!['coefficientTemperature']?.toStringAsFixed(1) ?? '0'}',
                        style: Theme.of(context).textTheme.bodyLarge,
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Puissance thermique totale : ${_resultat!['puissanceTotale']?.toStringAsFixed(1) ?? '0'} kW',
                        style: Theme.of(context).textTheme.bodyLarge,
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'COP estimé : ${_resultat!['cop']?.toStringAsFixed(1) ?? '0'}',
                        style: Theme.of(context).textTheme.bodyLarge,
                      ),
                      const SizedBox(height: 8),
                      Text(
                        'Puissance électrique nécessaire : ${_resultat!['puissanceElectrique']?.toStringAsFixed(1) ?? '0'} kW',
